  ...
  try {
    ...
    heroImage.innerHTML = `
            <div class="relative w-96 h-96 rounded-lg shadow-lg overflow-hidden" id="carousel">
                ...
                <!-- Navigation arrows -->
                <button onclick="carousel.prev()" class="absolute left-2 top-1/2 -translate-y-1/2 bg-white/50 hover:bg-white/75 rounded-full p-2">
                    <svg class="w-6 h-6 text-[#4F6F52]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </button>
                <button onclick="carousel.next()" class="absolute right-2 top-1/2 -translate-y-1/2 bg-white/50 hover:bg-white/75 rounded-full p-2">
                    <svg class="w-6 h-6 text-[#4F6F52]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
