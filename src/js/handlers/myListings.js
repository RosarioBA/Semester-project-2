...
export async function handleMyListings() {
    ...
    try {
        mainContent.innerHTML = '<div class="animate-spin rounded-full h-12 w-12 border-b-2 border-[#4F6F52] mx-auto mt-12"></div>';
        
        const { data: listings } = await getMyListings();

        const grid = document.createElement('div');
        grid.className = 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto px-4 py-8';
